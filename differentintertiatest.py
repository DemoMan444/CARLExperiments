...
class TrainingConfig:
    ...
    # Train on five narrow ranges centered on: 0.89, 1.0, 1.2, 1.5, 2.0
    ...
    eval_inertias: Tuple[float, ...] = (0.89, 1.0, 1.2, 1.5, 2.0)
...
class CARLMarioEnv(CARLEnv):
    ...
    def get_context_features() -> dict[str, ContextFeature]:
        # Env still uses these internal features, but only mario_inertia is observed.
        return {
            ...
            "mario_inertia": UniformFloatContextFeature(
                "mario_inertia", lower=0.89, upper=2.0, default_value=0.89
            ),
        }
...
class MarioObsAdapter(gym.ObservationWrapper):
    def __init__(
        self,
        env: gym.Env,
        ctx_keys: Sequence[str] = ("mario_inertia",),  # only inertia
...
def build_inertia_train_contexts(
    ...
    for map_idx in range(n_maps):
        ...
        for (low, high) in inertia_ranges:
            ...
            inertia = float(lo if hi <= lo else np.random.uniform(lo, hi))
...
def setup_training(config: TrainingConfig):
    ...
    try:
        ...
        extractor_cls = CombinedExtractor
    except ImportError:
    ...
    # SB3 will create a subfolder inside it (e.g., run_dir/tb_1)
    model = PPO(
